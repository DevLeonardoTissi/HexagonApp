package br.com.leonardo.hexagonapp.ui.screens.form.sections.userPhotoPicker

import br.com.leonardo.hexagonapp.ui.screens.form.PersonalProfileFormState
import br.com.leonardo.hexagonapp.ui.screens.form.PersonalProfileFormUIState
import br.com.leonardo.ui.content.layout.section.HexagonSection

class UserPhotoPickerSection :
    HexagonSection<PersonalProfileFormState, PersonalProfileFormUIState, UserPhotoPickerSectionRender>(
        view = UserPhotoPickerView(),
        mapper = UserPhotoPickerSectionMapper()
    )